...
const Scrollani = ({ children, reappear, threshold = 0.5 }: Props) => {
  ...
  return (
    <>
      <div ref={containerRef} className={`transition duration-[1100ms] ${isVisible ? "opacity-100 blur-none translate-x-0" : "opacity-0 blur-lg -translate-x-20"}  motion-reduce:transition-none motion-reduce:hover:transform-none`}>
        {children}
      </div>
    </>
  );
